  ...
  return (
    <section className="rounded-xl border border-brick-700/50 bg-brick-900/40 flex flex-col h-[min(70vh,36rem)] lg:h-[min(80vh,42rem)] lg:sticky lg:top-6">
      <div className="p-4 border-b border-brick-700/40 shrink-0">
        ...
        <p className="text-xs md:text-sm text-brick-300 mt-1">
          {projectName ? (
            <>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-3">
        {messages.length === 0 && (
          <p className="text-xs text-brick-300">
        ...
        {messages.map((m, i) => (
          ...
          >
            <span className="text-[10px] uppercase tracking-wide text-brick-300 block mb-1">
              {m.role}
            </span>
            <div className="whitespace-pre-wrap wrap-break-word">{m.content}</div>
          </div>
        ))}
        {pending && <p className="text-xs text-brick-300 animate-pulse">Thinking…</p>}
        <div ref={bottomRef} />
      </div>
